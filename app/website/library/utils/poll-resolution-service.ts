 ...
 */
export async function resolveWithAI(question: string, startDate?: number): Promise<ResolutionResult> {
  try {
...
${referenceDate ? `Poll creation date: ${referenceDate.toISOString().split('T')[0]}` : ''}
...
REASONING: [Brief explanation]`
    ...
    toast.error(`Error resolving poll: ${error}`);
...
function formatSearchQuery(question: string, referenceDate?: Date): string {
  ...
  if (!question.includes(" on ") && !question.includes(" in ") && !question.includes(" by ") && 
      !question.includes(" during ") && !question.includes(" at ")) {
 ...
 */
async function fallbackResolution(question: string, startDate?: number): Promise<ResolutionResult> {
  const referenceDate = startDate ? new Date(startDate) : undefined;
  const formattedQuery = formatSearchQuery(question, referenceDate);
  ...
  const confidence = totalResponses > 0 
    ? Math.max(yesCount, noCount) / totalResponses 
    : 0.5;
